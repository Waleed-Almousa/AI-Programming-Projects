...
if __name__ == "__main__":
    ...
    for i in range(EPISODES):
        ...
        while (not done):
            ...
            if not done: 
                Q_table[(obs, action)] = Q_table[(obs, action)] + LEARNING_RATE * (reward + DISCOUNT_FACTOR * max([Q_table[(current_obs, i)] for i in range(env.action_space.n)]) - Q_table[(obs, action)])
            else:
